...
# 🚀 투박한 배경색을 없애고, 하단 언더라인용 그라데이션 컬러만 심플하게 반환합니다.
def get_gauge_style(count):
    if count >= 1000000: return "linear-gradient(90deg, #b91c1c, #f87171)" # Gold 대신 강렬한 Red 계열
    elif count >= 200000: return "linear-gradient(90deg, #1d4ed8, #60a5fa)" # Blue 계열
    else: return "linear-gradient(90deg, #475569, #cbd5e1)" # Slate 계열
...
def generate_html():
    ...
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    ...
    .member-module {{ 
        ...
        border-radius: 4px; overflow: hidden; /* 바가 모서리를 뚫지 않도록 둥글게 깎음 */
        transform: translateZ(0);
        cursor: pointer; 
    ...
    /* 🚀 가독성을 방해하던 배경 막대를 없애고, 바닥에 깔리는 얇은 선으로 재탄생 */
    .member-bg-bar {{ 
        position: absolute; left: 0; bottom: 0; /* 맨 아래 바닥에 고정 */
        height: 3px; /* 게이지 선 두께 */
        border-radius: 0 2px 2px 0;
        box-shadow: 0 -2px 6px rgba(255, 255, 255, 0.15); /* 선 위쪽으로 은은한 빛 반사 */
    ...
    .member-content {{
        ...
        padding: 0 10px; z-index: 2; /* 텍스트는 선보다 위쪽에 떠있음 */
    }}
    ...
    /* 숫자 뒷배경이 깔끔하게 비워졌으므로 이제 방해 없이 완벽하게 잘 보입니다. */
    .count-main {{ 
    ...
    for c in final_data:
        ...
        for i, m in enumerate(c['members']):
            grad_color = get_gauge_style(m['v']['monthly'])
            ...
            <div class="member-module">
                <div class="member-bg-bar" style="width:{w}%; background: {grad_color};"></div>
...
if __name__ == "__main__":
    ...
    print("Success: 언더라인(하단바) 게이지 갱신 완료!")